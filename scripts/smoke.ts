...
async function main() {
  ...
  await fetchProfile(agent.agent.id);
...
async function fetchProfile(expectedAgentId: string) {
  ...
  const json = await res.json();
  assert.equal(res.status, 200, `profile fetch failed: ${JSON.stringify(json)}`);
  assert(json.profile, "profile response should include a row");
  assert.equal(json.profile.letta_agent_id, expectedAgentId, "profile must reference created agent");
}
